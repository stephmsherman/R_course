...
files= Sys.glob("~/Downloads/n_back_data/ndata_p??.csv")
all_nback=data.frame(c())
for (i in 1:length(files)){
	nback=read.csv(files[i])
	all_nback=rbind(all_nback,nback)
}
...
table(all_nback$participant)
table(all_nback$participant,all_nback$correct_response)
...
all_nback$correct_response==all_nback$actual_response
head(all_nback$correct_response==all_nback$actual_response, 20)
#Now create the column using the logical argument
all_nback$correct=ifelse(all_nback$correct_response==all_nback$actual_response, 1,0)
head(all_nback$correct, 20)
...
p=aggregate(correct~participant, all_nback, mean)
range(p$correct)
summary(p$correct)
...
h=aggregate(actual_response~participant, all_nback[all_nback$correct_response==1,], mean)
names(h)=c("participant","hit")
...
f=aggregate(actual_response~participant, all_nback[all_nback$correct_response==0,], mean)
names(f)=c("participant","false_alarm")
f
...
c$corrected_recognition=c$hit-c$false_alarm
c
...
#examples of barplot
barplot(c(1,2,3))
barplot(matrix(c(1,2,3,4),nrow=2))

participant=c$participant
y=c[,c("hit","false_alarm")]
barplot(t(y), col=c("grey","red"), xlab="Participant", names.arg=participant, ylab="Proportion of Responses", las=1, main="Hits and False Alarms for each participant")
legend(1.5, 1.2, legend=c("hit", "false alarm"), fill=c("grey", "red"),xpd=T, bty="n")
...
panas=read.csv("~/Downloads/PANAS.csv")
str(panas)
...
final=merge(c, panas, by="participant", all=F)
final
