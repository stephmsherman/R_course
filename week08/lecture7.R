...
g=read.csv("path/classroom_data.csv")
head(g)
str(g)
...
#main="title" 
hist(g$test_score,breaks=5, xlim=c(.6,1.1), ylim = c(0,35), main="Histogram of Test Scores")

#xlab (label on x axis) and ylab (label on y axis)
hist(g$test_score,breaks=5, xlim=c(.6,1.1),ylim = c(0,35),  main="Histogram of Test Scores", xlab="test scores", ylab="frequency")

#col ="colors"
hist(g$test_score,breaks=5, xlim=c(.6,1.1),ylim = c(0,35), main="Histogram of Test Scores", xlab="test scores", ylab="frequency",col="blue")
...
plot(first50$test_score, first50$hours_slept, pch=15, col="red", xlim=c(.6,1.01),ylim=c(5.2,8.5), xlab="Test Scores",ylab="Hours Slept")
#keeping the plot window from the first graph open 
#add a plot using the par function
par(new=TRUE)
#plot last 50 in blue
plot(last50$test_score, last50$hours_slept, pch=15, col="blue", xlim=c(.6,1.01),ylim=c(5.2,8.5), xlab="", ylab= "", xaxt='n', yaxt='n')
...
#if a participant received less than .8 on the test then we want their datapoint to be red otherwise it should be blue
#col=ifelse(g$test_score<.8,"red","blue")

plot(g$test_score,g$hours_slept, pch=15, col=ifelse(g$test_score<.8,"red","blue"), xlim=c(.6,1.01),ylim=c(5.2,8.5), xlab="Test Scores",ylab="Hours Slept")
#you can use ifelse function in most of the graphing parameters

#shape of data point
#not only do we want participants who receive less than .8 on the test to be blue but we also want the shape of the data point to be triangle, otherwise we want the datapoint to be a diamond
...
legend("bottomright", c("Failing Scores","Passing Scores"),pch=c(18,24), col=c("red", "blue"))
...
#add the parameters lwd and lty to the abline line. Set them at any number between 1 and 3 and see what happens
plot(g$test_score,g$hours_slept, pch=ifelse(g$test_score<.8,18,24), col=ifelse(g$test_score<.8,"red","blue"), xlim=c(.6,1.01),ylim=c(5.2,8.5), xlab="Test Scores",ylab="Hours Slept")
legend(.85,6, c("Failing Scores","Passing Scores"),pch=c(18,24), col=c("red", "blue"))
...
text(.93, 5.3, "r = .45, p < .000001, n = 95")
