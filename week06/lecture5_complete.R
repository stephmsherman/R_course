d=read.csv("~/Downloads/classroom_data.csv")
#check data
head(d)
str(d)
dim(d)
...
s=read.csv("~/Downloads/classroom_data_intervention.csv")
head(s)
str(s)
...
ptest=t.test(hours_slept~condition,s, paired=TRUE)
ptest
...
t.test(hours_slept~condition,s, paired=FALSE)
...
t.test(hours_slept~condition,s, paired=FALSE,var.equal=TRUE)
...
t.test(test_score~condition,s,paired=TRUE)
levels(s$condition)
##
...
write.csv(post,"~/Downloads/post.csv",row.names=FALSE)
