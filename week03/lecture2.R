...
#save your homework as a .R file
#DO NOT INCLUDE THE OUTPUT IN YOUR HOMEWORK, ONLY THE CODE. I AM GOING TO TAKE 1 POINT OFF ON EVERY ANSWER THAT INCLUDES OUTPUT FROM NOW ON 
#for example: if the answer to a question is data[1] only include data[1] not > data[1] or data[1]  [1] 5

#IMPORTING DATA INTO R

#save your data as a .csv or .txt NOT .xls or .xlsx
#I will demonstrate how to do that
...
#Download class_data.zip. Upzip the file and in the folder make sure you have: csv_data.csv,comma_delimited.txt, tab_delimited_data.txt, quiz_data.txt, tricky_data.txt, object_types.csv from Canvas and save it somewhere on your computer
...
## Windows users: go to the file "Properties" and copy the path from the "Location:" (Can also use .pdf in the modules for today)
...
#if so, add another argument : header = TRUE to your function
#if not: header = FALSE
...
head(data)#look at first few rows of your data (default is to look at 6 rows)
...
names(data3)
c("hours_slept","test_scores") #want to replace "V1" "V2" with these names
#make sure have names in same order 
names(data3)=c("hours_slept","test_scores")
#so all it did was take "V1" "V2" and replace it with "hours_slept" "test_scores"
...
#Note: sometimes you can get this error when you have the file open that you are trying to import into R
